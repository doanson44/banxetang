...
public class RightMenu extends GameEntity {
	...
	public static void UpdateText() {
		...
		if (Player1 != null)
			Player1.detachSelf();
		Player1 = new Text(GameManager.MAP_SIZE + 20,
				GameManager.MAP_SIZE - 200, GameManager.getFont("font2"),
	...
	public void AddStateText() {
		Text State = new Text(-150, 0, GameManager.getFont("font2"), "STATE "
				+ GameManager.getCurrentStage(), GameManager.VertexBufferObject);
	...
	protected void InitEnermyTank(float px, float py) {
		...
		for (int i = 0; i < _TotalEnermy; i++) {
			EnermyIcon icon = new EnermyIcon(px
					+ (i % 2 * (EnermyIcon.GetSize() + 5)), pyStart + py);
			_mEnermys.add(icon);
