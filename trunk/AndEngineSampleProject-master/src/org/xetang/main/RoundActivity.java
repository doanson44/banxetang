...
public class RoundActivity extends Activity implements OnClickListener {
	...
	private void initRootView() {
		...
		text.setTypeface(Typeface.createFromAsset(getAssets(), "font/font2.ttf") );
		text.setText("Select Stage");
		text.setTextColor(Color.WHITE);
		text.setTextSize(64f);
		root.addView(text, 0, new LayoutParams(LayoutParams.MATCH_PARENT, titleHeight));
		...
		horizontalView.setLayoutParams(new LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT));
	...
	private void calcFrameSize() {
		...
		this.frameWidth = (display.widthPixels - (NUM_COLUMN + 1) * this.paddingWidth) / NUM_COLUMN;
		this.frameHeight = (display.heightPixels - titleHeight - (NUM_ROW + 1) * this.paddingHeight) / NUM_ROW;
		this.frameStartX = 0;
		this.frameStartY = 0;
	...
	private void addRoundView(final Round r) {
		...
		//int x = frameStartX + numPage * display.widthPixels + paddingWidth + (r.Index-1) % NUM_COLUMN * frameWidth;
		//int y = frameStartY + (r.Index-1) / NUM_COLUMN * frameHeight + paddingHeight;
		...
		try {
			...
			image.setImageBitmap( BitmapFactory.decodeStream(getAssets().open(r.ImageAssert)) );
			layout.addView(image, new RelativeLayout.LayoutParams( RelativeLayout.LayoutParams.MATCH_PARENT, RelativeLayout.LayoutParams.MATCH_PARENT));
			
			Button b = new Button(this);
			b.setText("Stage " + r.Index);
			RelativeLayout.LayoutParams paramsButton = new RelativeLayout.LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT);
			paramsButton.setMargins(0, layout.getHeight()/2 - b.getHeight()/2, 0, 0);
	...
	private void loadRounds() {
		try {
			String[] names = getAssets().list("round");
			
			for (String name : names) {
				if(!name.contains("default")){
					int idx = Integer.parseInt(getNamePart(name));
					rounds.add(new Round(GameManager.mReachedStage>=idx?"round/"+name:"round/"+DEFAUILT_IMAGE, idx , GameManager.mReachedStage >= idx));
				}
			}
